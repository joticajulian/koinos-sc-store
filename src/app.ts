...
export class KoinosSCMicroservice {
  ...
  processPendingBlock(block: BlockJSON) {
    block.transactions.forEach((tx) => {
      tx.active_data.operations.forEach((operation) => {
        if (
          operation.type === Operation.KoinosProtocolUploadContractOperation
        ) {
          ...
          if (
            this.contractsAccepted.find(
              (c) =>
                c.data.block.id === block.id && c.data.transaction.id === tx.id
            )
          ) {
            return;
          }
  ...
  async setBlockIrreversible(blockIrreversible: BlockIrreversibleJSON) {
    ...
    this.contractsAccepted = this.contractsAccepted.filter((c) => {
      return (
        c.data.block.height !== blockIrreversible.topology.height ||
    ...
    if (this.contractsAccepted.length > 0) {
      log(
        `There are ${
          this.contractsAccepted.length
        } contracts accepted in the queue from blocks: ${this.contractsAccepted.map(
          (c) => c.data.block.height
        )}`
      );
    }
    ...
    if (blockIrreversible.topology.height > db.getBlockHeight() + 1) {
      ...
      log(
        `Calling blockstore because db.blockHeight is ${db.getBlockHeight()} and amqp is in block ${
          blockIrreversible.topology.height
        }`
      );
      this.syncronize();
      return;
    ...
    if (blockIrreversible.topology.height < db.getBlockHeight() + 1) {
      log(
        `Warning: irreversible block ${
          blockIrreversible.topology.height
        } already in the database (height: ${db.getBlockHeight()}). Ignoring block`
      );
      return;
    }
  ...
  async syncronize() {
    ...
    let diffBlocks =
      (this.irreversibleBlocksProcessed[0].height as number) - blockNumber;
    while (diffBlocks > 0) {
    ...
    if (diffBlocks < 0) {
      ...
      this.irreversibleBlocksProcessed.sort(
        (a, b) => (a.height as number) - (b.height as number)
      );
      this.irreversibleBlocksProcessed =
        this.irreversibleBlocksProcessed.filter((b) => {
          return b.height >= blockNumber;
        });
    }

    while (
      this.irreversibleBlocksProcessed.length > 0 &&
  ...
  async updateDatabase(topology: BlockTopologyJSON) {
    ...
    if (
      this.contractsAccepted.find((c) => c.data.block.height <= topology.height)
    ) {
      // there are pending items from old blocks, even older than the irreversible
      // block stored in the database
      const oldContracts = this.contractsAccepted.filter(
        (c) => c.data.block.height <= topology.height
      );
      log(
        `fatal: ${oldContracts.length} contracts are still in the pending list. They will be removed`
      );
      this.contractsAccepted = this.contractsAccepted.filter(
        (c) => c.data.block.height > topology.height
      );
    }
...
export default KoinosSCMicroservice;
